...
impl AbstractOp {
    ...
    pub(crate) fn realize(&self, address: u32) -> Result<Self, TryFromIntError> {
        let ret = match self {
            ...
            Self::Macro(_, _) => panic!("macros cannot be realized"),
        };
    ...
    pub(crate) fn concretize(self) -> Option<Op<Concrete>> {
        let res = match self {
            ...
            Self::Macro(_, _) => panic!("macros cannot be concretized"),
        };
...
impl fmt::Display for AbstractOp {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Self::Macro(name, content) => write!(f, r#"%{}: {:?}"#, name, content),
        }
    }
}
