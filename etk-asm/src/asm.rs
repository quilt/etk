...
impl Assembler {
    ...
    /// initially estimated, the function adjusts the code accordingly. This step is essential for
    /// ensuring that the generated bytecode is correct and adheres to the EVM's requirements for
    /// push instructions.
                    ...
                    if let AbstractOp::Push(imm) = op {
                        ...
                        if !labels.is_empty() {
                            if let Ok(val) = exp {
                                ...
                                let extra = (val.to_f64().unwrap().log2() / 8.0).ceil() as usize;

                                // If the size of the push operation has changed (we already reserved
                                // size for a push1), we need to adjust the code accordingly.
                                if extra > 1 {
                                    self.concrete_len += extra - 1;

                                    for label in labels {
                                        // Update the position of the label to account for the extra bytes.
                                        if let Some(position) = self.declared_labels.get_mut(&label)
                                        {
                                            // As the label's value is based on concrete_len, we need to
                                            // add the extra bytes to the label's position.
                                            *position = position.map(|v| v + extra - 1);
                                        }
                                    }
                                }
                            }
